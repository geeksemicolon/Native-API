...
class LoginM extends CI_Model{
  ...
  // function untuk insert data ke tabel mhs_login
  public function add_login($nama,$email,$password){
    ...
    if(empty($nama) || empty($email) || empty($password)){
      ...
      $check = $this->db->get('mhs_login');
      if ($check->num_rows() < 1){
          $insert = $this->db->insert("mhs_login", $data);
          if($insert){
            $response['status']=200;
  ...
  public function all_login(){
    ...
    $all = $this->db->get("mhs_login")->result();
    $response['status']=200;
    $response['error']=false;
    $response['login']=$all;
    return $response;
  ...
  public function auth_login($email,$password){
    $login = $this->db->get_where('mhs_login',array('email'=>$email));
    if ($login->num_rows() > 0) {
  ...
  public function delete_login($id){
    ...
    if($id == ''){
      ...
      $this->db->where($where);
      $delete = $this->db->delete("mhs_login");
      if($delete){
        $response['status']=200;
        $response['error']=false;
  ...
  public function update_login($id,$nama,$email){
    ...
    if($id == '' || empty($nama) || empty($email)){
      ...
      $this->db->where($where);
      $update = $this->db->update("mhs_login",$set);
      if($update){
        $response['status']=200;
        $response['error']=false;
