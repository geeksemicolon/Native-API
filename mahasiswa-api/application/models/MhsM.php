...
class MhsM extends CI_Model{
  ...
  // function untuk insert data ke tabel mhs_mhs
  public function add_mhs($nim,$nama,$jurusan){
    ...
    if(empty($nim) || empty($nama) || empty($jurusan)){
      ...
      $insert = $this->db->insert("mhs_mhs", $data);
  ...
  public function all_mhs(){
    ...
    $all = $this->db->get("mhs_mhs")->result();
    $response['status']=200;
    $response['error']=false;
    $response['mhs']=$all;
    return $response;
  ...
  public function get_mhs_by_id($id){
    ...
    $mhs = $this->db->get_where('mhs_mhs', array('id' => $id))->result();
    $response['status']=200;
    $response['error']=false;
    $response['mhs']=$mhs;
    return $response;
  ...
  public function update_mhs($id,$nim,$nama,$jurusan){
    ...
    if($id == '' || empty($nim) || empty($nama) || empty($jurusan)){
      ...
      $this->db->where($where);
      $update = $this->db->update("mhs_mhs",$set);
      if($update){
        $response['status']=200;
        $response['error']=false;
  ...
  public function foto/mhs($id, $foto){
      if($id == '' || empty($foto)){
          return $this->empty_response();
      } else {
          $path = $_SERVER['DOCUMENT_ROOT'].str_replace(basename($_SERVER['SCRIPT_NAME']),"",
						$_SERVER['SCRIPT_NAME'])."foto/mhs/".$id.".jpeg";
            $finalpath = base_url()."foto/mhs/".$id.".jpeg";
